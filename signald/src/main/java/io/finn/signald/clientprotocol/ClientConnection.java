...
public class ClientConnection implements Runnable {
    ...
    private void handleError(PrintWriter w, Throwable error, String requestId) {
        ...
        try {
            JsonMessageWrapper message = new JsonMessageWrapper("unexpected_error", new JsonStatusMessage(0, error.getMessage()), requestId);
            send(w, message);
        } catch (IOException e) {
            logger.catching(e);
        }
            ...
            try {
                ...
                try {
                    ...
                    if (version.equals("v0")) {
                        throw new IllegalArgumentException("v0 requests are no longer supported");
                    } else {
